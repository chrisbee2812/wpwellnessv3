...
import { services, serviceCategories, getImageById, Service, ServiceCategory } from '@/lib/data';
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
          <TabsList className={`grid w-full ${gridColsClass} md:w-[800px] mx-auto mb-10`}>
            {allCategories.map((category) => (
              <TabsTrigger key={category} value={category as string}>
                {category}
              </TabsTrigger>
            ))}
          </TabsList>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {services
                  .filter((service) => {
                    ...
                    return service.primaryCategory === category || service.secondaryCategories?.includes(category as ServiceCategory)
                  })
